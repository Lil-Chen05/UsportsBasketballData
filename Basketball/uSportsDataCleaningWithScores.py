...
usports_sched = pd.read_csv("USportsSched_2024-25.csv", header=None) # INCLUDE SEASON SCHEDULE FILE HERE!!
...
usports_clean['Year'] = np.where(
     usports_clean['Month'].isin(['July', 'August', 'September', 'October','November', 'December']),
     2024,
...
usports_clean['Away_Score'] = (
    usports_clean['Away']
    .str.extract(r'(\d+)$')[0]  # extract first capturing group
    .astype(float)              # float can hold NaN
)

usports_clean['Home_Team'] = usports_clean['Home'].str.replace(r'\d+$', '', regex=True).str.strip()
usports_clean['Home_Score'] = (
    usports_clean['Home']
    .str.extract(r'(\d+)$')[0]   # extract first regex group
...
def determine_home_away(row):
    ...
    mask_home = games_today['Home_Team'] == team
    if mask_home.any():
        g = games_today.loc[mask_home].iloc[0]
        return pd.Series({
            'HomeAway': 'Home',
            'Opponent': g['Away_Team'],
            'TeamPoints': g['Home_Score'],
            'OppPoints': g['Away_Score']
        })

    mask_away = games_today['Away_Team'] == team
    if mask_away.any():
        g = games_today.loc[mask_away].iloc[0]
        return pd.Series({
            'HomeAway': 'Away',
            'Opponent': g['Home_Team'],
            'TeamPoints': g['Away_Score'],
            'OppPoints': g['Home_Score']
        })

    return pd.Series({'HomeAway': pd.NA, 'Opponent': pd.NA, 'TeamPoints': pd.NA, 'OppPoints': pd.NA})

jerrysFile[['HomeAway','Opponent','TeamPoints','OppPoints']] = (
    jerrysFile.apply(determine_home_away, axis=1)
)


# Export cleaned data
jerrysFile.to_csv("2024-25DataCleanFinalWithScores.csv", index=False) # INSERT DESIRED EXPORT FILE NAME HERE!
